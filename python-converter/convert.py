...
#-------------------------------------------------------------------------
# SDM: added compile options here

# set following to True to push document revisions as git commits
# if False, no git commands will be executed (useful when testing)
COMMIT_TO_GIT = False

# enable to add Yaml header to markdown files (if using Jekyll)
# No need for Yaml in our use case
WRITE_YAML = False

# Enable hard line breaks option for MD conversion. Seems to break links & paragraphs, so disabled
PANDOC_HARDLINEBREAKS = False

# Enable use of the page title as the output filename (better for Github since that uses filenames in the TOC/Page List)
USE_TITLE_AS_FILENAME = True

# force wikilinks to use relative urls (ie. all in same folder)
ENABLE_RELATIVE_WIKILINKS = True

# enable fixing of wiki assets (png/gif/jpg/zip) so they are referenced via the assets_prefix folder
FIX_ASSET_LINKS = True
assets_prefix = "./images/"

# enable fixing of underscores or underlines, for some reason pandoc is emitting an extra line return
FIX_HEADERS = True

#-------------------------------------------------------------------------

# define the base folder where pages are stored
prefix = "wiki/"
# define the base wikilink URL, for Github wikis this can be empty as all pages are relative links
wikilink_prefix = ""
...
git_command = "git" # assume on path
pandoc = "c:\\program files (x86)\\pandoc\\pandoc.exe" # assume on path
...
def cleanup_markdown(text, source_url):
    ...
    for old in p.findall(text):
        ...
        if ENABLE_RELATIVE_WIKILINKS == True:
            new = "](%s%s" % (wikilink_prefix, old[2:])
        else:
            new = "](/%s%s" % (prefix, old[2:])
        #print("Replacing %s --> %s" % (old[1:], new[1:]))
        text = text.replace(old, new)

    if FIX_ASSET_LINKS == True:
        # match any markdown links that are links to some kind of asset
        #p = re.compile('(]\([A-Za-z0-9._%+-]+\.(?i)(jpg|png|zip|gif)+)')
        p = re.compile('(?i)(]\(.+\.png |]\(.+\.jpg |]\(.+\.zip |]\(.+\.gif )')
        
        for old in p.findall(text):
            if old.startswith(("](http", "](ftp:", "](mailto:")):
                continue

            new = "](%s%s" % (assets_prefix, old[2:])
            # remove any "media:" tags/prefixes
            new = new.replace("Media:", "")
            new = new.replace("_", " ") # stupid pandoc replaces spaces in filenames with underscores, so undo this. 

            print "Matched '" + old + "', replacing with '" + new + "'"
            text = text.replace(old, new)       


        # regex to find <img .. /> elements and replace with markdown references
        if "<img src" in text:
            print "Found <img src"

            p = re.compile("(?i)(<img src=\"([a-z0-9.-_]+)\".+\/>)")
            ...
            for match in matches:
                ...
                old = match[0]
                new = "![](" + assets_prefix + match[1] +")"
                new = new.replace("_", " ") # stupid pandoc replaces spaces in filenames with underscores, so undo this. 

                text = text.replace(old, new)
                print "<IMG> replaced '" + old + "' with '" + new + "'"
...
def make_url(title):
    ...
    if USE_TITLE_AS_FILENAME == True:
        return os.path.join(prefix, title.replace(" ", " ") + "/")
    else:
...
def make_filename(title, ext):
    ...
    """
    if USE_TITLE_AS_FILENAME == True:
        return os.path.join(prefix, title.replace(" ", " ").replace(":", " ").replace("/", " ") + os.path.extsep + ext)
    else:
...
def commit_files(filenames, username, date, comment):
    ...
    # early out if no Git versions of documents are required
    if COMMIT_TO_GIT == False:
        return 

    #cmd = '"%s" add "%s"' % (git_command, '" "'.join(filenames))
    cmd = '%s add "%s"' % (git_command, '" "'.join(filenames))
    run(cmd)
    ...
    #       % (git_command, filename, date, author, comment)
    cmd = [git_command, 'commit'] + filenames + [
                              '--date', date,
                              '--author', author,
                              '-F', '-',
                              '--allow-empty']
...
for title, filename, date, username, text, comment in c.execute('SELECT * FROM revisions ORDER BY date, title'):
    ...
    if dump_revision(mw_filename, md_filename, text, title):
        ...
        if COMMIT_TO_GIT == True:
            cmd = git_command + " reset --hard"
            run(cmd)
...
if COMMIT_TO_GIT == True:
    ...
    run("%s filter-branch --prune-empty -f HEAD" % git_command)
